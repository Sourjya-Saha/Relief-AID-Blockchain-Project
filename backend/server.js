// server.js - Relief Aid System Backend (Vercel-Compatible)
...
app.get('/', (req, res) => {
  res.json({
    ...
    message: 'Relief Aid System Backend is live on Vercel',
    timestamp: new Date().toISOString(),
    ...
    endpoints: {
      ...
      redemptionRequest: 'POST /api/redemption/request',
      redemptionProof: 'POST /api/redemption/proof',
      ipfsFetch: 'GET /api/ipfs/:cid'
    }
  });
});
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    timestamp: new Date().toISOString(),
    pinataConnected: PINATA_API_KEY ? true : false,
    endpoints: {
      health: 'GET /api/health',
      testPinata: 'GET /api/test-pinata',
      categoryToEnum: 'GET /api/category/to-enum/:category',
...
app.post('/api/merchant/upload-profiles-batch', async (req, res) => {
  try {
    ...
    for (let i = 0; i < profiles.length; i++) {
      ...
      if (!profile.name || !profile.phone || !profile.address || !profile.walletAddress || !profile.category) {
        errors.push({
          index: i,
          error: 'Missing required fields'
        });
